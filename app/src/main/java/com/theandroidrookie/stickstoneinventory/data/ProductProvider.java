...
public class ProductProvider extends ContentProvider {
     ...
     * @return Cursor object which can given to {@link com.theandroidrookie.stickstoneinventory.ProductCursorAdapter}
     */
    @Nullable
    @Override
    ...
    private void sanityCheck(ContentValues values) {
        ...
        if (values.containsKey(ProductContract.ProductEntry.COLUMN_PRODUCT_NAME)) {
            String suppName = values.getAsString(ProductContract.ProductEntry.COLUMN_PRODUCT_SUPPLIER_NAME);
            if (suppName == null) {
                throw new IllegalArgumentException("Product requires supplier name");
            }
        }
        if (values.containsKey(ProductContract.ProductEntry.COLUMN_PRODUCT_NAME)) {
            String suppEmail = values.getAsString(ProductContract.ProductEntry.COLUMN_PRODUCT_SUPPLIER_EMAIL);
            if (suppEmail == null) {
                throw new IllegalArgumentException("Product requires supplier email");
            }
        }
        if (values.containsKey(ProductContract.ProductEntry.COLUMN_PRODUCT_NAME)) {
            String suppPhone = values.getAsString(ProductContract.ProductEntry.COLUMN_PRODUCT_SUPPLIER_PHONE);
            if (suppPhone == null) {
